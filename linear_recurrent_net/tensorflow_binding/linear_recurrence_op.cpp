...
REGISTER_OP("SerialLinearRecurrence")
    .Input("decays: float")
    .Input("impulses: float")
    .Input("initial_state: float")
    .Output("response: float")
...
class GpuSerialLinearRecurrenceOp : public OpKernel {
public:
  explicit GpuSerialLinearRecurrenceOp(OpKernelConstruction *ctx): OpKernel(ctx) {}
  ...
  void Compute(OpKernelContext *ctx) override {
    ...
    compute_serial_linear_recurrence(decays.data(), impulses.data(),
                  initial_state.data(), response.data(),
                  n_dims, n_steps);
  }
};
REGISTER_KERNEL_BUILDER(Name("SerialLinearRecurrence").Device(DEVICE_GPU), GpuSerialLinearRecurrenceOp);
