...
REGISTER_OP("FastLinearRecurrence")
    .Input("decays: float")
...
class GpuFastLinearRecurrenceOp : public OpKernel {
public:
  explicit GpuFastLinearRecurrenceOp(OpKernelConstruction *ctx): OpKernel(ctx) {}
  ...
  void Compute(OpKernelContext *ctx) override {
    ...
    compute_fast_linear_recurrence(decays.data(), impulses.data(),
                  initial_state.data(), response.data(),
                  n_dims, n_steps);
  }
};
REGISTER_KERNEL_BUILDER(Name("FastLinearRecurrence").Device(DEVICE_GPU), GpuFastLinearRecurrenceOp);


REGISTER_OP("LinearRecurrenceBaseline")
    .Input("decays: float")
    .Input("impulses: float")
    .Input("initial_state: float")
    .Output("response: float")
...
class GpuLinearRecurrenceBaselineOp : public OpKernel {
public:
  explicit GpuLinearRecurrenceBaselineOp(OpKernelConstruction *ctx): OpKernel(ctx) {}
  ...
  void Compute(OpKernelContext *ctx) override {
    ...
    compute_linear_recurrence_baseline(decays.data(), impulses.data(),
			      initial_state.data(), response.data(),
			      n_dims, n_steps);
  }
};
REGISTER_KERNEL_BUILDER(Name("LinearRecurrenceBaseline").Device(DEVICE_GPU), GpuLinearRecurrenceBaselineOp);


REGISTER_OP("SerialLinearRecurrenceBaseline")
    .Input("decays: float")
    .Input("impulses: float")
    .Input("initial_state: float")
    .Output("response: float")
...
class GpuSerialLinearRecurrenceBaselineOp : public OpKernel {
public:
  explicit GpuSerialLinearRecurrenceBaselineOp(OpKernelConstruction *ctx): OpKernel(ctx) {}
  ...
  void Compute(OpKernelContext *ctx) override {
    ...
    compute_serial_linear_recurrence_baseline(decays.data(), impulses.data(),
                  initial_state.data(), response.data(),
                  n_dims, n_steps);
  }
};
REGISTER_KERNEL_BUILDER(Name("SerialLinearRecurrenceBaseline").Device(DEVICE_GPU), GpuSerialLinearRecurrenceBaselineOp);
